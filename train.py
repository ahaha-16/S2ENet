...
from models.mglnetconc import MYNet_2 as BaseNet
...
def val(args, val_loader, model, epoch):
    ...
    for iter, batched_inputs in enumerate(val_loader):
        ...
        loss = BCEDiceLoss(output4, target_var) + BCEDiceLoss(output3, target_var) + BCEDiceLoss(output2, target_var) + \
...
def train(args, train_loader, model, optimizer, epoch, max_batches, cur_iter=0, lr_factor=1.):
    ...
    for iter, batched_inputs in enumerate(train_loader):
        ...
        # run the model
        output, output2, output3, output4 = model(pre_img_var, post_img_var)
        loss = loss = BCEDiceLoss(output4, target_var) + BCEDiceLoss(output3, target_var) + BCEDiceLoss(output2, target_var) + \
               BCEDiceLoss(output, target_var)
...
def trainValidateSegmentation(args):
    ...
    args.savedir = args.savedir + '_' + args.file_root + '+mynet' + '/'
    args.vis_dir = args.savedir + '/Vis/'

    if args.file_root == 'LEVIR':
        args.file_root = ' '   #your dataset path
...
if __name__ == '__main__':
    ...
    parser.add_argument('--file_root', default="CLCD", help='Data directory | LEVIR | WHU | SYSU ')
